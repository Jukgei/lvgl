...
void lv_gauge_set_value(lv_obj_t * gauge, uint8_t needle_id, int32_t value)
{
    ...
    else if(ext->needle_imgs) {
        lv_img_header_t info;
        lv_img_decoder_get_info(ext->needle_imgs[needle_id].needle_img, &info);
        needle_w = info.h;
    }
    else {
    ...
    if(ext->needle_img || ext->needle_imgs) {
        needle_angle = (old_value - min) * angle / (max - min) + angle_ofs;
        needle_angle += 180;
        if (ext->needle_img)
            r = LV_MATH_MAX(ext->needle_img_pivot.x, ext->needle_img_pivot.y);
        else
            r = LV_MATH_MAX(ext->needle_imgs[needle_id].needle_img_pivot.x, ext->needle_imgs[needle_id].needle_img_pivot.y);
...
static void lv_gauge_draw_needle(lv_obj_t * gauge, const lv_area_t * clip_area)
{
    ...
    if(ext->needle_img == NULL && ext->needle_imgs == NULL) {
    ...
    for(i = 0; i < ext->needle_count; i++) {
        ...
        /*Draw image*/
            ...
            {
                lv_img_header_t info;
                lv_img_decoder_get_info(ext->needle_imgs[i].needle_img, &info);

                lv_area_t a;
                a.x1 = x_ofs - ext->needle_imgs[i].needle_img_pivot.x;
                a.y1 = y_ofs - ext->needle_imgs[i].needle_img_pivot.y;
                a.x2 = a.x1 + info.w - 1;
                a.y2 = a.y1 + info.h - 1;
                img_dsc.pivot.x = ext->needle_imgs[i].needle_img_pivot.x;
                img_dsc.pivot.y = ext->needle_imgs[i].needle_img_pivot.y;
                ...
                if (needle_angle > 3600) needle_angle -= 3600;
                img_dsc.angle = needle_angle;
                lv_draw_img(&a, clip_area, ext->needle_imgs[i].needle_img, &img_dsc);
            }
